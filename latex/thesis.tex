...
A second design choice, parallel to the reward, concerns what the agent perceives at each step. Many environments expose only their current state, which carries positional information, but not motion. Whether an approaching object is moving toward or away from the agent, for instance, cannot be inferred from a single snapshot. The standard remedy is to give the agent a short history of recent observations rather than only the most recent one, an approach known as frame stacking \citep{Mnih2015}. With recent history available, the agent can in principle learn to infer derived quantities, such as velocity, directly from its input.
...
\paragraph{Shelters.} 
Stationary tiles generated at episode start with density 0.01 per cell (approximately 41 shelters per episode). While in a shelter, the agent takes no damage from enemies. Shelters don't protect against starvation.
...
Table~\ref{tab:outcomes} reports the per-cell episode-outcome composition and mean shelter occupancy. The composition is visualised in Figure~\ref{fig:failure_modes} (left panel) and the shelter-occupancy column in the right panel; this table provides the exact percentages for reference. The movement composition is in Table~\ref{tab:movement}.
